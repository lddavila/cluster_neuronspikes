function [table_with_sorted_channels] = determine_best_dimensions_of_data(table_of_best_representation,updated_table_of_overlap,config)
%this version does it based on snr and we can already tell it isn't good, so best not to use it
table_with_best_channels = table(nan(size(table_of_best_representation,1),1), ...
    ...
    cell(size(table_of_best_representation,1),1) ...
    ,'VariableNames',["Z Score","Tetrode","Cluster","Sorted Channels","Sorted SNR"]);
