function [updated_table_of_other_appearences] = update_table_of_overlap_ver_2(old_table_of_other_appearences,config)
...
if config.ON_HPC
    neural_net_struct = importdata(config.FP_TO_MERGABLE_OR_NOT_NN_ON_HPC);
else
    neural_net_struct = importdata(config.FP_TO_MERGABLE_OR_NOT_NN);
end
...
for i=1:size(updated_table_of_other_appearences,1)
    ...
    for k=1:length(other_appearences_overlap_percentage_array)
        ...
        c_1 = current_classification~="1";
        c_2 =current_overlap_percentage < config.OVERLAP;
        c_3 = current_euc_dist_between_waveforms < config.MAX_EUC_DIST;

        

        merge_or_dont_nn = neural_net_struct.net;
        is_mergable = predict(merge_or_dont_nn,[current_cluster_waveform,other_app_mean_waveform]);
        [~,index_of_max] = max(is_mergable);
        is_mergable_by_nn = index_of_max-1;

        if (config.ONLY_OVERLAP_WITH_NEURONS && c_1 )|| c_2|| c_3 || ~is_mergable_by_nn
            other_appearences_clusters_array(k) = NaN;
            other_appearences_tetrode_array(k) = "";
            other_appearences_overlap_percentage_array(k) = "";
            other_appearences_z_score_array(k) = NaN;
