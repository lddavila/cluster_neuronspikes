...
for i=1:size(number_of_accuracy_categories,2)
    ...
    parfor j=1:size(number_of_layers,2)
        ...
        for k=1:size(filter_sizes,2)
            ...
            name_to_save_under = "accuracy score "+num2str(accuracy_score)+" number of acc cats " +string(number_of_accuracy_cats)+" num layers "+string(num_layers)+ " filter size "+string(current_filter_size);
