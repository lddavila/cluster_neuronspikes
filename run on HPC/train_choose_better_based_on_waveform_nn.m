function [] = train_choose_better_based_on_waveform_nn()
home_dir =cd("..");
addpath(genpath(pwd));
cd(home_dir);
...
for i=1:size(random_samples,1)
    ...
    wave_1_accuracy = updated_table_of_overlap{wave_1_idx,"accuracy"};
    wave_2_idx = current_row(2);
    wave_2 = updated_table_of_overlap{wave_2_idx,"Mean Waveform"}{1};
    
    wave_2_accuracy = updated_table_of_overlap{wave_2_idx,"accuracy"};
    if wave_1_accuracy>= wave_2_accuracy
        wave_1_is_better = 1;
    else
        wave_1_is_better =0;
    end
    data_for_nn{i,:} = [wave_1,wave_2,wave_1_is_better];
    if mod(i,1000)==0
