...
for i=1:length(list_of_tetrodes)
    ...
    for z_score_counter=1:length(list_of_z_scores_to_check)
        ...
        dir_with_grades = dir_of_nth_pass_recording+"\initial_pass_results min z_score"+string(current_z_score)+" grades\";
        ...
        load(dir_with_grades+current_tetrode+".mat");
    ...
    array_of_pannels =[1,4,7,10;2,5,8,11;3,6,8,12];
    array_of_heatmap_panels = [13 14 15];
    ...
    for first_dimension = 1:length(channels_in_current_tetrode)
        ...
        for second_dimension = first_dimension+1:length(channels_in_current_tetrode)
            for z_score_counter=1:length(list_of_z_scores_to_check)
                current_z_score = list_of_z_scores_to_check(i);
                ...
                y_values = strcat("cl",string(1:size(grades,1)));
                heatmap(x_values,y_values,relevant_grades_of_current_tetrode);
                title("Grades")
            end
