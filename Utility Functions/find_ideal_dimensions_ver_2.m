function [rows_to_use,table_of_ideal_dimensions] = find_ideal_dimensions_ver_2(current_table_of_channels,min_improvement,min_amp)
%the purpose of this version is to return a list of all channels that meet
%the desired criteria and allow find_ideal_dimensions.mat to choose the
%best ones from there
channel_counter =1;
rows_to_use =nan(1000,1);
...
for i=1:size(consecutive_ranges,1)
    ...
    for j=1:size(table_of_current_channel,1)-1
        ...
        if improvement < min_improvement && next_SNR>current_SNR %indicates that the SNR is no longer improving in step sizes greater than the min_improvement threshold
...
rows_to_use(isnan(rows_to_use)) = []; %remove any unused dimensions
