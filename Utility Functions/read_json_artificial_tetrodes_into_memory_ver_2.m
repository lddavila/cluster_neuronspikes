function [array_of_return_dictionaries] = read_json_artificial_tetrodes_into_memory_ver_2(path_of_json_files)
...
%the sixth dictionary is samples_format_of_map_version_of_struct
    %this dictionary is the same thing as spike_tetrode dictionary, but reformatted so that it is structured differently
        %1.)
        %2.)
        %3.)
...
for i=1:length(list_of_json_files)
    ...
    for j=1:length(list_of_keys)
        ...
        if i~=1 && i~=2
            ...
            if i~=2
                ...
                map_version_of_struct("Nothing") = current_struct;
            end
        end
    ...
    disp(string(i)+" Finished " + list_of_json_files(i))
end
