function [] = compare_before_after_filter_application(dir_with_grades,dir_with_outputs,relevant_grades,names_of_relevant_grades,spikesort_config,the_range)
art_tetr_array = build_artificial_tetrode();
list_of_tetrodes = strcat("t",string(1:285));
number_of_filters_to_apply = length(relevant_grades);
for i=1:length(list_of_tetrodes)
    ...
    for first_dimension = 1:length(channels_of_curr_tetr)
        for second_dimension = first_dimension+1:length(channels_of_curr_tetr)
            subplot(number_of_filters_to_apply,6,plot_counter);
    ...
    % subplot(number_of_filters_to_apply,6,7:12);
