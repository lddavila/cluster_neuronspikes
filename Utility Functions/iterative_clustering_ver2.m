function clusters = iterative_clustering_ver2(aligned, ir, tvals, refine_spike_idx, subsets, config,extract_features_fn)
...
%   subsets, config) returns the final clusters.
    ...
    for k = 1:length(subsets)
        ...
        [cf, bad_tmp] = run_clustering_ver_2(aligned, filtered_idx, ir, tvals, refine_spike_idx, config,extract_features_fn);
        ...
        if config.DO_BAD_CLUSTER_ROUND && ~isempty(bad)
            ...
            cf_bad = run_clustering_ver_2(aligned, filtered_idx, ir, tvals, refine_spike_idx, config,extract_features_fn);
        else
            cf_bad = {};
        end
