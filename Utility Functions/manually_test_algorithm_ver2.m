function [has_been_computed] = manually_test_algorithm_ver2(scale_factor,dir_with_channel_recordings,min_z_score,num_dps,timestamps_dir,precomputed_dir,what_is_pre_computed,min_threshold,size_of_tetrode,art_tetr_array,example_name,extract_features_fn,dir_to_save_figs_to)
...
% filesnames = split(filenames,"\");
% filesnames = strjoin(filenames(1:end));
if ~isempty(output_array{1})
    ...
    for first_dimension = 1:length(art_tetr_array)
        for second_dimension = first_dimension+1:length(art_tetr_array)
            ...
            % saveas(gcf,dir_to_save_figs_to+"\"+string(first_dimension) +" vs "+string(second_dimension)+".fig");
            % close(gcf)
