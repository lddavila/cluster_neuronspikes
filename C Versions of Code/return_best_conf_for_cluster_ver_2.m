...
for i=1:size(table_of_overlapping_clusters,1)
    current_z_score = table_of_overlapping_clusters{i,1};
    current_cluster_num = table_of_overlapping_clusters{i,2};
    current_overlap_percentages = split(table_of_overlapping_clusters{i,3},"|");
    current_other_tetrode_appearences = split(table_of_overlapping_clusters{i,5},"|");
    current_cluster_other_appearence = split(table_of_overlapping_clusters{i,4},"|");
    ...
    if current_overlap_percentages==""
        ...
        if  ~isempty(already_appeared)
            if ~ismember(table_of_other_appearences{1,"Tetrode"}+" "+string(table_of_other_appearences{1,"Z Score"})+" "+string(table_of_other_appearences{1,"Cluster"}),already_appeared)
