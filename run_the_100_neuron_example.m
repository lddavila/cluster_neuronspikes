...
[best_appearences_of_cluster_2,timestamps_of_best_clusters_2,table_of_overlapping_clusters_2]= id_best_representation_of_clusters(varying_z_scores,tetrodes_to_check,min_overlap_percentage,debug,grades_that_matter,names_of_grades,generic_dir_with_grades,generic_dir_with_outputs,dir_to_save_figs_to,load_previous_attempt,save_results,time_delta,refinement_pass,dir_to_save_to);
original_location_of_the_array = 1:size(best_appearences_of_cluster,1);
%%
best_appearences_of_cluster_2 = best_appearences_of_cluster(best_appearences_of_cluster{:,4}>.0,:);
timestamps_of_best_clusters_2 = timestamps_of_best_clusters(best_appearences_of_cluster{:,4}>.0);
...
desired_unit = 78;
table_of_blind_pass_accuracy_of_clusters = table_of_accuracy_of_clusters(table_of_accuracy_of_clusters{:,"unit_id"}==desired_unit,:);
grades_to_check = ["overlap_with_unit","accuracy"];
create_heatmaps_of_grades_in_accuracy_table(table_of_blind_pass_accuracy_of_clusters,grades_to_check);
...
%% run the reclustering, beginning with the situations where clusters repeat then expanding out
for i=1:9
clc;
close all;
load_previous_attempt = true;
save_current_attempt= true;
dir_to_save_data_to = pwd+"\Second Pass Data";
...
min_improvment = .1;
number_of_channels_in_new_config = i;
num_dps = 60;
...
scale_factor = -1;
min_threshold = 20;
recluster_with_ideal_dimensions(table_of_overlapping_clusters,load_previous_attempt,save_current_attempt,dir_to_save_data_to,version_name,generic_dir_with_grades,generic_dir_with_outputs,precomputed_dir,min_improvment,number_of_channels_in_new_config,num_dps,dir_with_chan_recordings,timestamps,scale_factor,channel_wise_means,channel_wise_std,min_threshold)
end
...
dir_to_save_grades_to = create_a_file_if_it_doesnt_exist_and_ret_abs_path(dir_to_save_grades_to);
list_of_tetrodes = strcat("t",string(1:192));
dir_with_timestamps_and_rvals = "D:\spike_gen_data\Recordings By Channel Precomputed\0_100Neuron300SecondRecordingWithLevel3Noise\refinement_pass";
name_of_grades = ["Tight","% Short ISI","Inc", "Temp Mat","Min Bhat","Skewness","TM Updated","Sym of Hist","Amp Category"];
relevant_grades = [2,3,4,8,9,28,29,30,31];
get_grades_for_nth_pass_of_clustering(dir_with_timestamps_and_rvals,dir_with_output,list_of_tetrodes,dir_to_save_grades_to,config,varying_z_scores(i),debug,relevant_grades,name_of_grades)
...
%% 
refined_pass_table_of_accuracy_of_clusters = table_of_accuracy_of_clusters_2;
grades_to_check = ["overlap_with_unit","accuracy"];
create_heatmaps_of_grades_in_accuracy_table(refined_pass_table_of_accuracy_of_clusters,grades_to_check,50);
%%
desired_unit=78;
refined_pass_table_of_accuracy_of_clusters = table_of_accuracy_of_clusters_2(table_of_accuracy_of_clusters_2{:,"unit_id"}==desired_unit,:);
grades_to_check = ["overlap_with_unit","accuracy"];
create_heatmaps_of_grades_in_accuracy_table(refined_pass_table_of_accuracy_of_clusters,grades_to_check);
