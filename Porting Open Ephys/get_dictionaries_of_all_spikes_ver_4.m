function [all_spike_tetrode_dicts] = get_dictionaries_of_all_spikes_ver_4(art_tetr_array,spike_windows,dir_with_chan_recordings,timestamps,number_of_dps_per_slice,scale_factor,dictionaries_dir)
...
all_spike_tetrode_dicts = cell(1,size(art_tetr_array,1));
for i=1:size(art_tetr_array,1)
    ...
    [current_slice,current_timing,current_spiking_channels,spike_slices_in_samples_format] = get_slices_per_artificial_tetrode_ver_3(channels_in_current_tetrode,spike_windows,dir_with_chan_recordings,timestamps,number_of_dps_per_slice,scale_factor);


    try
        spike_tetrode_dictionary("t"+string(i)) = current_slice;
        all_spike_tetrode_dicts{i} = spike_tetrode_dictionary;
        ...
        % disp("Couldn't Make t"+string(i)+ " Cause the file was too big to save")
    end
end
end