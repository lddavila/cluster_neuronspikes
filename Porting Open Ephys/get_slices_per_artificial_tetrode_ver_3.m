...
all_90th_percentile = nan(1,length(chan_of_art_tetrode));
...
for i=1:length(chan_of_art_tetrode)
    ...
    if isempty(fomatted_channel_peaks)
        channel_peaks{i} = [];
        continue;
    end
    ...
    all_90th_percentile(i) =nintyeth_percentile;
end
...
for i=1:length(chan_of_art_tetrode)
    ...
    if isnan(channel_means(i)) || isnan(all_90th_percentile(i))
        continue;
    end
    for j=1:size(modified_spikes{i},1)
        ...
        if isempty(modified_spikes{i}(j))
            continue;
        end 
        if isnan((modified_spikes{i}(j,4)))
            continue;
        end
        

        if channels_data{i}(modified_spikes{i}(j,4)) < all_90th_percentile(i) 
            continue;
        end
        % disp(channels_data{i}(spike_windows{current_channel}{j}(4)))
        spike_windows_for_current_channel(j,1) = modified_spikes{i}(j,1);
        spike_windows_for_current_channel(j,2) = modified_spikes{i}(j,2);
        spike_windows_for_current_channel(j,3) = modified_spikes{i}(j,3);
        spike_windows_for_current_channel(j,4) = modified_spikes{i}(j,4);
